-- 수정 전 장기 동기화가 다시 생성한 조이찬 중복 담당자 노드를 기존 계정 노드로 병합한다.
DO $$
DECLARE
  v_keep_id uuid := '40605438-9fc8-4dac-acda-f8b37c3add5b';
  v_duplicate_id uuid := '236294ab-c230-47c9-a9a6-3fb6ebd8862c';
BEGIN
  IF NOT EXISTS (
    SELECT 1
    ...
    RAISE EXCEPTION '유지할 조이찬 계정 노드의 식별 정보가 예상과 다릅니다.';
  END IF;
  ...
  IF EXISTS (
    ...
    WHERE id = v_duplicate_id
      AND (name <> '조이찬' OR external_id IS NOT NULL OR source_customer_id IS NOT NULL)
  ) THEN
  ...
  IF EXISTS (
    ...
    RAISE EXCEPTION '중복 조이찬 노드에 0이 아닌 정산 데이터가 있어 병합을 중단합니다.';
  END IF;
