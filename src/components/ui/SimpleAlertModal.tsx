...
/** 삭제·푸시 결과 등 관리자 화면용 간단 알림 모달 */
export default function SimpleAlertModal({ open, variant, title, message, onClose }: Props) {
  if (!open) return null;
  ...
  return (
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="simple-alert-title"
    >
      ...
      <div className="relative w-full max-w-sm rounded-2xl bg-white p-6 shadow-xl ring-1 ring-slate-900/10">
        ...
        <h2 id="simple-alert-title" className="mt-4 text-center text-lg font-bold text-slate-900">
          {title}
        </h2>
