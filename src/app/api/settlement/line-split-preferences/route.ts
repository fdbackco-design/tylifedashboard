 ...
 * - GET  /api/settlement/line-split-preferences?year_month=YYYY-MM
 * - PUT  /api/settlement/line-split-preferences  { year_month, top_line_id, is_split }
 */
...
export async function PUT(req: NextRequest): Promise<NextResponse> {
  let body: { year_month?: string; top_line_id?: string; is_split?: boolean } = {};
  try {
    body = await req.json();
  } catch {
  ...
  const isSplit = body.is_split;

  if (!isYearMonth(yearMonth)) return NextResponse.json({ error: 'year_month(YYYY-MM) 필요' }, { status: 400 });
  if (!topLineId || !isUuid(topLineId)) return NextResponse.json({ error: 'top_line_id(UUID) 필요' }, { status: 400 });
  if (typeof isSplit !== 'boolean') return NextResponse.json({ error: 'is_split(boolean) 필요' }, { status: 400 });

  const db = createAdminSupabaseClient();
  const { error } = await db
    .from('settlement_line_split_preferences')
    .upsert(
      { year_month: yearMonth, top_line_id: topLineId, is_split: isSplit, updated_at: new Date().toISOString() } as any,
  ...
  return NextResponse.json({ success: true, year_month: yearMonth, top_line_id: topLineId, is_split: isSplit });
}
